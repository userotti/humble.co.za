...
(function($){
	...
	$.fn.baldrick.registerhelper('bootstrap_modal', {
		event	: function(el){
			...
			if(triggers.data('modal')){
				if(triggers.data('modal') !== 'true'){
					modal_id = triggers.data('modal');
				}
				if(!$('#' + modal_id + '_baldrickModal').length){

					// fadeout existing modals
					var existing = $('.modal-dialog');

					if(existing.length){
						$('.modal-dialog').animate({'opacity': .3}, 200);
						bm_hasModal = true;
					}
		...
		params	: function(params,defaults){
			...
			if(trigger.data('modal') && (params.url || trigger.data('modalContent'))){
					...
					if(buttons.length){
						for(b=0; b<buttons.length;b++){
							...
							if(callback === 'dismiss'){
								...
								button.attr("onclick", callback + "(this); ");
							}
				...
				modal.modal('show').on('hidden.bs.modal', function (e) {
					if(bm_hasModal){
						$('.modal-dialog').animate({'opacity': 1}, 200);
					}
